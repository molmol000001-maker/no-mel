...
// framer-motion UMD の取り出し（環境によってグローバル名が揺れるのを吸収）
const MotionGlobal = window.Motion || window.motion || window['framer-motion'] || {};
const motion = MotionGlobal.motion || ((p)=>React.createElement('div', p));
...
function App(){
  ...
  // 100段階スコア（校正済み）
  // 75kg・男性がビール500ml(5%)を飲んだ直後 targetScore=43 で約30分目安
  ...
  const burnRate = useMemo(() => {
    let v = sex === "male" ? 7.2 : sex === "female" ? 6.8 : 7.0; // g/h
  ...
  // ソフトドリンク（追加1杯ごとに -10分、直後の必須1杯はボーナスなし）
  ...
  // 直近飲酒の最小クールダウン（20g→30分を比例）
  const minCooldownSec = lastAlcoholTs
    ? Math.max(0, Math.round((lastDrinkGrams / 20) * 1800) - Math.floor((Date.now() - lastAlcoholTs) / 1000))
    : 0;

  // 直近60分の摂取量に基づくフレンドリー上限
  const gramsRecent60 = (() => {
    const cutoff = Date.now() - 60*60*1000;
    let sum = 0;
    for (const h of history) {
      if (h.type==='alcohol' && h.ts>=cutoff) {
        sum += gramsOfAlcohol(Number(h.abv||0), Number(h.ml||0));
      }
    }
    return sum;
  })();
  ...
  // ヘッダ酔い度（タイマー0で飲酒履歴ありの時は targetScore までクリップ表示）
  const headerScoreExact = useMemo(()=>{
    return (hasAnyAlcohol && displaySec===0) ? Math.min(scoreExact, targetScore) : scoreExact;
  ...
  const needsWater = history.length>0 && history[0].type==='alcohol';
  ...
  const addWater = () => {
    ...
    setLastWaterTs(now);
    if (!mandatory) setWaterBonusSec(s=> s + 600); // 追加水は10分短縮
  };
  const handleWaterClick = () => {
    ...
    setTimeout(()=>{ addWater(); showToast("ソフトドリンクを記録しました"); setSpin(false); }, 700);
  };
  const endSession = () => {
  ...
  useEffect(()=>{
    try{
      console.assert(Math.abs(gramsOfAlcohol(5,350)-14) < 0.05, "g(5%,350ml) ≈ 14g");
      console.assert(Math.abs(gramsOfAlcohol(5,500)-20) < 0.05, "g(5%,500ml) ≈ 20g");
      console.assert(fmtMMSS(0) === "0:00", "fmt 0:00");
      // header clamp
      const clamp = (se, any, sec, tgt)=> (any && sec===0) ? Math.min(se, tgt) : se;
      console.assert(clamp(60,true,0,43)===43, "header clamp ok");
      // soft drink bonus clamp
      (function(){ const base=300, bonus=600; const after=Math.max(0, base - Math.min(bonus, base)); console.assert(after===0,'bonus clamp'); })();
    }catch(e){}
  },[]);
    ...
    <div className="min-h-[100dvh] w-full text-slate-900 bg-gradient-to-b from-slate-50 to-slate-100">
      <div className="max-w-md mx-auto relative flex flex-col min-h-[100dvh]">
        ...
        <main className="flex-1 px-4 pb-32 pt-3">
          <section className="bg-white rounded-2xl p-4 shadow-sm grid gap-4" style={{ display: "main"===tab ? "grid" : "none" }}>
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold tracking-tight">{fmtMMSS(hasAnyAlcohol ? nextOkSec : 0)}</div>
                {hasAnyAlcohol && nextOkSec>0 && <div className="text-[11px] text-slate-500">（{fmtTime(Date.now()+nextOkSec*1000)} 目安）</div>}
                {effectiveBonusSec>0 && <div className="text-[11px] text-slate-500">ソフトドリンク効果: -{Math.floor(effectiveBonusSec/60)}分</div>}
              </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {DRINKS.cocktail.options.map((op, idx)=>(
                      <button key={op.label}
                        ...
                        onClick={()=>setOptionIdx(idx)}>{op.label}</button>
                    ))}
                  </div>
                </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {DRINKS[cat].options.map((op, idx)=>(
                          <button key={op.label}
                            className={`h-11 px-3 rounded-xl text-sm font-semibold border active:scale-[.98] ${optionIdx===idx?'bg-slate-900 text-white':'bg-white'}`}
                            onClick={()=>setOptionIdx(idx)}>{op.label || '既定量'}</button>
                        ))}
                      </div>
                    </div>
        ...
        {/* ソフトドリンク オーバーレイ */}
        <AnimatePresence>
          {history.length>0 && history[0].type==='alcohol' && (
            <motion.div key="gate"
              ...
              <motion.div className="bg-white rounded-2xl w-full max-w-xs p-6 text-center shadow-xl"
                ...
                <div className="flex flex-col items-center relative">
                  ...
                  <AnimatePresence>
                    {showSparks && [{x:0,y:-60,size:18},{x:40,y:-45,size:14},{x:60,y:-5,size:16},{x:55,y:35,size:14},{x:15,y:60,size:16},{x:-20,y:60,size:14},{x:-55,y:30,size:16},{x:-60,y:-5,size:14},{x:-35,y:-45,size:16},{x:20,y:-60,size:14}].map((s,i)=>(
                      <motion.span key={`sp1-${burstKey}-${i}`} className="absolute select-none"
                        style={{left:'50%',top:'50%',marginLeft:-8,marginTop:-8,fontSize:s.size,color:['#60a5fa','#34d399','#fbbf24','#f472b6','#f87171'][i%5]}}
