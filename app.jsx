// ===== Framer Motion UMD フォールバック =====
const FM = window.framerMotion || window.FramerMotion || {};
const motion = FM.motion || ((props) => <div {...props} />);
...
function App() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("alc_waterBonusSec", String(waterBonusSec));
    } catch (e) {}
  }, [waterBonusSec]);
  useEffect(() => {
    try {
      localStorage.setItem("alc_lastAlcohol", String(lastAlcoholTs));
    } catch (e) {}
  }, [lastAlcoholTs]);

  const [lastDrinkGrams, setLastDrinkGrams] = useState(Number(localStorage.getItem("alc_lastDrinkGrams") || 0));
  useEffect(() => {
    try { localStorage.setItem("alc_lastDrinkGrams", String(lastDrinkGrams)); }
    catch (e) {}
  }, [lastDrinkGrams]);
  ...
  // header 表示の見かけ酔い度（飲酒可能=タイマー0のときは targetScore までクリップ）
        ...
        <main className="flex-1 px-4 pb-32 pt-3">
              ...
              {ORDER.map((id) => (
                <button
                  ...
                  className={`h-11 px-3 rounded-xl text-sm font-semibold border active:scale-[.98] ${
                    cat === id ? 'bg-slate-900 text-white' : 'bg-white'
                  }`}
                  onClick={() => {
                    setCat(id);
                    setOptionIdx(0);
            ...
            {cat === 'cocktail' ? (
              <>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {DRINKS.cocktail.options.map((op, idx) => (
                      <button
                        key={op.label}
                        className={`h-11 px-3 rounded-xl text-sm font-semibold border active:scale-[.98] ${
                          optionIdx === idx ? 'bg-slate-900 text-white' : 'bg-white'
                        }`}
                ...
                <div className="bg-slate-50 border rounded-xl p-3 text-xs text-slate-600">
                  {optionIdx === 0 && (
                    <p>弱め（10度前後）：カシスオレンジやファジーネーブルなど、ジュースベースの飲みやすいカクテル。</p>
                  )}
                  {optionIdx === 1 && (
                    <p>レギュラー（20度前後）：ウォッカ、ジン、テキーラ、ラムなどのスピリッツをベースに他の材料を加えて作られます。</p>
                  )}
                  {optionIdx === 2 && (
                    <p>強め（30度前後以上）：マティーニ、マンハッタン、モヒートなど、スピリッツ比率が高め。</p>
                  )}
                </div>
                ...
                <div className="mt-2">
                  <button
                    ...
                    className={`w-full h-12 px-4 rounded-xl font-semibold active:scale-[.98] ${
                      needsWater ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-slate-900 text-white'
                    }`}
                    onClick={addDrink}
                  >
                    この内容で追加
                ...
                {needsWater && (
                  <div className="text-[11px] text-rose-600">一杯お酒を飲んだら、一杯ソフトドリンクを挟んでください。</div>
                )}
              </>
            ) : (
              <>
          ...
          <section className="bg-white rounded-2xl p-4 shadow-sm grid gap-4" style={{ display: tab === "settings" ? "grid" : "none" }}>
            ...
            <div className="grid gap-2">
              <button type="button" onClick={endSession} className="w-full h-12 px-4 rounded-xl border font-semibold active:scale-[.98]">
                飲み会終了
              </button>
            </div>
            <div className="text-[10px] text-slate-500">Reactプレビュー版 · データはこの端末のブラウザに保存されます（localStorage）</div>
          </section>
        </main>
